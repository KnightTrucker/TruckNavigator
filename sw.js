/* KnightTruckerNavy 0.70.44 STARTUP-FIX — non-blocking service worker */
const CACHE='ktn-007044-startup-layer-v1';
const CORE=[
  './',
  './index.html',
  './navigator.html',
    ...
    await Promise.all(keys.filter(k=>
      k!==CACHE && (
        ...
        (
          ...
          k.startsWith('ktn-007042-startup-fix-') ||
          k.startsWith('ktn-007043-bootstrap-diag-')
        )
      )
...
self.addEventListener('fetch',event=>{
  ...
  /* CDN e servizi esterni:
     lascia fare direttamente al browser. */
  if(url.origin!==self.location.origin) return;

  /* Media/Range:
     il browser gestisce direttamente 206/Range.
     Non passare dalla cache. */
  if(
  ...
  /* Asset locali:
     cache on demand;
     niente precache da 4.5 MB che blocca installazione. */
  event.respondWith(
    caches.match(
      event.request,
      {ignoreSearch:true}
